...
const secondarySubjectObjects = type => {
	return {
		//Flags
		'13': so => [],
		//Images
...
export const subjectData = {
	...
	checkedIn: (subjectObject, options = {}) => {
		...
		const subjectObjectSpecsUser = subjectObject.subjectType === globals.USER
		//with options.date and subjectObject specifies a user
		//if there is an active date associated with the user return the id
		if (subjectObjectSpecsUser && (options.date || options.eventType === globals.DATE)) {
			//console.log('subjectData checkedIn', subjectObject, options)
		...
		} else if (subjectObjectSpecsUser && (options.day || options.eventType === globals.DAY)) {
			//console.log('subjectData checkedIn', subjectObject, options)
			const lastCheckin = remoteData.Messages.lastCheckin(subjectObject.subject)
			//console.log('subjectData lastCheckin', lastCheckin)
		...
		} else if (subjectObjectSpecsUser && (options.set || options.eventType === globals.SET)) {
			//console.log('subjectData checkedIn', subjectObject, options)
			const lastCheckin = remoteData.Messages.lastCheckin(subjectObject.subject)
			//console.log('subjectData lastCheckin', lastCheckin)
	...
	places: (subjectObject) => {
		...
		return subjectData.festivals(subjectObject)
			.map(festival => { return { subject: festival.id, subjectType: globals.FESTIVAL } })
	...
	getReviews: (subjectObject, userId) => {
		...
		const allPrimaryReviewMessages = relatedMessages
			.filter(m => m.messageType === globals.RATING || m.messageType === globals.COMMENT)
