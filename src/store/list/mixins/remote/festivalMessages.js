...
export default (festivals, lineups) => {return {
	loadForFestival (festivalId) {
			...
			const filters = subjects.map(s => {return {
				where: {and: [
					s,
					{deleted: false}
				]},
			...
			const ends = filters.map(filter => `/api/Messages/?filter=${JSON.stringify(filter)}`)
			return Promise.all(ends.map(end => this.acquireListSupplement('', end)))
				.then(() => this.getFiltered(m => {
					const okSet = m.subjectType !== SET || setIds.includes(m.subject)
					const okDay = m.subjectType !== DAY || dayIds.includes(m.subject)
					const okDate = m.subjectType !== DATE || dateIds.includes(m.subject)
					const okFestival = m.subjectType !== FESTIVAL || m.subject === festivalId
					const okSeries = m.subjectType !== SERIES || m.subject === fest.series
					const okArtist = m.subjectType !== ARTIST || artistIds.includes(m.subject)
					return okSeries && okDay && okDate && okFestival && okSeries && okArtist
				}))
				.then(baseMessages => {
					const baseFilter = {
						where: {and: [
