...
const DiscussionWidget = {
	...
	view: ({ attrs }) => <LargeCardWidget
		header="Discuss"
		headerCard={attrs.headerCard && attrs.messageArray.length && (attrs.messageArray.some(m => m.messageType && m.subjectType !== globals.MESSAGE) || attrs.messageArray.some(m => m.flagType)) ?
			subjectCard(
				attrs.messageArray.find(m => m.flagType || m.subjectType !== globals.MESSAGE),
	...
	>
		...
		}
		{
			...
			attrs.messageArray.some(f => f.flagType) ?
				subjectCard(
					{ subjectType: globals.FLAG, subject: attrs.messageArray.find(f => f.flagType).id },
					flagAttrs(attrs)
				) : ''
		}
		{
					...
					buildTree(
						...
						attrs.messageArray.some(f => f.flagType) ?
							...
							remoteData.Messages.getFiltered(
								m => m.subjectType === globals.MESSAGE && attrs.messageArray.map(me => me.id).includes(m.baseMessage)
							)
					) : {}
			)
