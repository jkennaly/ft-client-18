...
const rawPlaces = (subjectObject, count = 5) => {
	...
	const placeSearchStrings = places
		.map(s => { return { subject: s.id, subjectType: globals.PLACE } })
		...
		view: ({ attrs }) => <div>
			...
			<WidgetContainer>
				...
				<FixedCardWidget header={`Check In`} display={true}>
					{// my current checkin
						`Current checkin: ${subjectData.name(subjectData.checkedIn({ subject: attrs.userId, subjectType: globals.USER }))}`
					}
				...
				<FixedCardWidget header={`Locations`} display={true}>
					{
						subjectData.places(attrs.subjectObject)
							...
							.map(s => subjectCard({ subject: s.id, subjectType: globals.PLACE }, { dayId: attrs.dayId }))
				...
				<FixedCardWidget header={`People`} display={true}>
					{
						...
						subjectData.users(attrs.subjectObject)
							.map(s => {
								const mainSubject = { subject: s.id, subjectType: globals.USER }
