...
const extraction = (() => {
    ...
    return elNumber => extracted => {
        if(_.isUndefined(extracted)) return hidden.reduce((total, cv) => total || cv, false)
	...
	view: ({attrs}) => <div class={classes(attrs)}>
        <div class="ft-modal-content">
        {
            ...
            <header class={`ft-modal-info-plate ${extraction()() ? 'c44-dn' : ''}`}>
                <p>Current FestiBucks:  <i class="fas fa-coins" />{attrs.bucks}</p>
                <p>Date Access: <i class="fas fa-coins" />3</p>
            </header>
          {
