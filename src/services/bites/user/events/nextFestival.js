...
const festivalIntentions = (goerId, intentions, festivals) => intentions
	.lbfilter({
		where: { and: [
				{user: goerId},
	...
	.then(intendedFestivalIds => intendedFestivalIds.map(x => x.subject).sort(festivalIdsByEndTimeSort(festivals)))
	.then(intendedFestivalIds => intendedFestivalIds[0] && festivals.getLocalPromise(intendedFestivalIds[0]) || [false, false])
	.then(([fest, upd]) => fest)
	.then(fav => {
...
const intendedFestival = (goerId, intentions, festivals) => {
	...
	if (!cacheOk) festivalIntentions(goerId, intentions, festivals)
		.catch(console.log)
...
export default  (goerId, intentions, festivals) => {
	...
	const value = baseValue ? subjectCard({subject: baseValue.id, subjectType: FESTIVAL}, {
		userId: goerId,
		uiClass:''
