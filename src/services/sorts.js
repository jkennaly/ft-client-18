...
const artistScore = (messages, artists, lineups, artistPriorities, festivals) => {
	return {
		friendActivity: () => 0,
		followedActivity: () => 0,
...
export const reviewSorter = interactions => viewer => sortArray => _.flatten(
	sortArray
		...
		.reduce((pv, cv, i, arr) => {
			...
			const followed = !user && interactions.some(int => int.type === globals.FOLLOW && int.subjectType === globals.USER && int.subject === cv.fromuser)
